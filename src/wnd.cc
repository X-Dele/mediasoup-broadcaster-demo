...
gboolean OnKeyPressCallback(GtkWidget* widget, GdkEventKey* key, gpointer data)
{
	reinterpret_cast<GtkMainWnd*>(data)->OnKeyPress(widget, key);
	return false;
}

void OnRowActivatedCallback(
  GtkTreeView* tree_view, GtkTreePath* path, GtkTreeViewColumn* column, gpointer data)
{
...
void InitializeList(GtkWidget* list)
{
	...
	GtkTreeViewColumn* column =
	  gtk_tree_view_column_new_with_attributes("List Items", renderer, "text", 0, NULL);
...
GtkMainWnd::GtkMainWnd(const char* server, int port, bool autoconnect, bool autocall)
  : window_(NULL), draw_area_(NULL), vbox_(NULL), server_edit_(NULL), port_edit_(NULL),
    peer_list_(NULL), callback_(NULL), server_(server), autoconnect_(autoconnect),
    autocall_(autocall)
{
	char buffer[10];
...
bool GtkMainWnd::Create()
{
	...
	if (window_)
	{
		...
		gtk_window_set_default_size(GTK_WINDOW(window_), 1366, 768);
...
void GtkMainWnd::SwitchToStreamingUI()
{
	...
	gtk_window_set_title(GTK_WINDOW(window_), "Streaming");
...
void GtkMainWnd::OnRedraw()
{
	...
	if (remote_renderer && remote_renderer->image() != NULL && draw_area_ != NULL)
	{
		...
		if (local_renderer && local_renderer->image())
		{
			...
			scaled -= 10;
			// ... towards the bottom.
			scaled += (height_ * width_ * 4) -
			          ((local_renderer->height() / 2) * (local_renderer->width() / 2) * 4);
			// bottom margin...
			scaled -= (width_ * 2) * 5;
...
GtkMainWnd::VideoRenderer::VideoRenderer(GtkMainWnd* main_wnd, webrtc::VideoTrackInterface* track_to_render)
